...
func (c *Checkbox) Add(label string, cell *entity.Cell, prop *props.Checkbox) {
	...
	if prop.Checked {
		...
		// The checkmark is composed of two segments:
		//   - a short stroke going from the left side down to the inner bottom
		//   - a longer stroke going from the inner bottom up to the upper right
		// Coordinates are expressed as fractions of prop.Size so the mark
		// scales with the checkbox.
		startX := x + prop.Size*0.2
		startY := y + prop.Size*0.5
		midX := x + prop.Size*0.45
		midY := y + prop.Size*0.75
		endX := x + prop.Size*0.85
		endY := y + prop.Size*0.25
