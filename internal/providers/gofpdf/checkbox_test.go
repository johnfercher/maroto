...
func TestCheckbox_Add(t *testing.T) {
	...
	t.Run("when label is empty and checkbox is checked, should draw border rect and checkmark lines", func(t *testing.T) {
		...
		// short stroke: from (0.2*size, 0.55*size) to (0.45*size, 0.8*size)
		fpdf.EXPECT().Line(2.0, 5.5, 4.5, 8.0)
		// long stroke: from (0.45*size, 0.8*size) to (0.8*size, 0.2*size)
		fpdf.EXPECT().Line(4.5, 8.0, 8.0, 2.0)
