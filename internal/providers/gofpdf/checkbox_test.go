...
func TestCheckbox_Add(t *testing.T) {
	...
	t.Run("when label is empty and checkbox is checked, should draw border rect and check mark lines", func(t *testing.T) {
		t.Parallel()
		// Arrange
		...
		// check mark: start (2.5, 6.25) → mid (3.75, 7.5)
		fpdf.EXPECT().Line(2.5, 6.25, 3.75, 7.5)
		// check mark: mid (3.75, 7.5) → end (8.75, 2.5)
		fpdf.EXPECT().Line(3.75, 7.5, 8.75, 2.5)
	...
	t.Run("when label is set and checkbox is checked, should draw border rect, check mark lines and label text", func(t *testing.T) {
		t.Parallel()
		// Arrange
		cell := &entity.Cell{X: 0, Y: 0}
		prop := &props.Checkbox{
		...
		// x = 0, y = 0
		fpdf.EXPECT().Rect(0.0, 0.0, 10.0, 10.0, "D")
		// check mark: start (2.5, 6.25) → mid (3.75, 7.5) → end (8.75, 2.5)
		fpdf.EXPECT().Line(2.5, 6.25, 3.75, 7.5)
		fpdf.EXPECT().Line(3.75, 7.5, 8.75, 2.5)
		// labelX = 0 + 10 + 1 = 11
		// labelY = 0 + 5 + 2 = 7
