...
| `Hyperlink` | `*string` | `nil` | URL — makes the text a clickable link (defaults to blue, can be overridden via `Color`) |

## Usage notes

- When `Hyperlink` is set, the text color defaults to blue. Set `Color` to override the link color (e.g. to match a brand style guide).
