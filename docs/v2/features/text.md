...
| `Hyperlink` | `*string` | `nil` | URL — makes the text a clickable link (rendered in blue by default) |

## Usage notes

- When `Hyperlink` is set, the text color defaults to blue. Setting `Color` overrides that default so links can match a custom style guide.
