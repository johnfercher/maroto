...
The Checkbox component renders a square checkbox with an optional label to its right. When `Checked` is true, a check mark (✓) is drawn inside the box using two connected line segments. This component is useful for forms, questionnaires, and agreements.
...
|-------|------|---------|-------------|
| `Checked` | `bool` | `false` | Whether the checkbox is marked with a check (✓) |
